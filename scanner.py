...
class Scanner:
    ...
    def main_scan_routine(self, url):
        ...
        if scan_result[0]:
            ...
            self.RESULT[domain].update({"xss_result":
                                             {"vul_url": url,
                                              "vul_parameter": scan_result[1]
                                              }
                                         }
                                )
        self.LOGGER.info("[+] XSS Scanner successfully processed url: %s" % str(url))

    def execute(self):
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_url = {executor.submit(Scanner.main_scan_routine, self, url): url for url in self.URL_LIST}
            #future_to_url = {executor.submit(Scanner.main_scan_routine, self, 'http://www.vfmii.com/exc/aspquery?command=invoke&ipid=1050372&ids=40704')}
